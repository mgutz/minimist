...
func TestOthers(t *testing.T) {
	...
	assert.Contains(res.Others(), "a")
	assert.Contains(res.Others(), "b")
	assert.Equal(2, len(res.Others()))

	// cmd -a b c d
	res = ParseArgv([]string{"-a", "b", "c", "d"})
	assert.Equal("b", res.MustString("a"))
	assert.Contains(res.Others(), "c")
	assert.Contains(res.Others(), "d")
	assert.Equal(2, len(res.Others()))
...
func TestOthersString(t *testing.T) {
	argm := ParseArgv([]string{"--zoom=1.0", "--", "one two"})
	argu := ParseArgv(argm.Unparsed())
	assert.Equal(t, "one two", argu.Others()[0])
}
